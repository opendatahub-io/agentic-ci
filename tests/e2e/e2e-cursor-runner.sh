...
# e2e-cursor-runner.sh -- End-to-end tests for the cursor-runner image
# using agentic-ci.
#
...
#
# Usage:
#   ./tests/e2e/e2e-cursor-runner.sh
...
cleanup() {
    agentic-ci stop --harness cursor 2>/dev/null || true
    rm -rf "$TMPDIR_E2E"
    echo ""
...
IMAGE="${CURSOR_CONTAINER_IMAGE:-localhost/cursor-runner:latest}"

if [[ -z "${CURSOR_API_KEY:-}" ]]; then
    echo ""
    print_warning "Skipping Cursor e2e tests (CURSOR_API_KEY is not set)"
    print_warning "Set CURSOR_API_KEY to run cursor harness e2e"
    exit 0
fi

print_step "Using CURSOR_API_KEY authentication"
...
print_step "Running Cursor via agentic-ci (streaming)..."
RC=0
...
print_step "Running Cursor via agentic-ci (non-streaming)..."
RC=0
agentic-ci run "Reply with only the word pong" \
    --image "$IMAGE" \
    --harness cursor \
...
print_step "Running Cursor with setup steps (podman)..."
...
print_step "Running Cursor with AGENTIC_CI_SKIP_SETUP=1 (podman)..."
...
print_warning "Cursor does not support plugins, skipping plugin filtering test"
print_success "PASS: Plugin test skipped (expected behavior for Cursor)"
PASS=$((PASS + 1))